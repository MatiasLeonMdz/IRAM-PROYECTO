...
## HISTORIA COMPLETA — v1.0
### Cubre: proyecto completo v1 → v5.5 | Generado: 2026-06-10 22:30
### Fuentes: IRAM_SUPERBACKUP_v2_1.md (v1→v4.0) + IRAM_BACKUP_ESTRATEGICO_v4_1_a_v5_5.md (v4.1→v5.5)

---

> **NOTA DE FUSIÓN:** Este documento unifica el SUPERBACKUP v2.1 (historia v1→v4.0) y el BACKUP ESTRATÉGICO (período v4.1→v5.5).
> Para desarrollo activo usar zip v5.5 + TECHNICAL_WIKI_ACTIVE v3.10.
> Este documento es registro histórico completo + insumo para Plantilla D, B, C.
...
## ── PARTE II: BACKUP ESTRATÉGICO — Historia v4.1 → v5.5 ──
...
## 1.2 Qué se implementó en v4.x (2026-05-22 → 2026-05-30) — GAP CERRADO

> ✅ **Gap v4.1→v4.3.16 cerrado el 2026-06-11** mediante análisis sistemático de los 5 conversations.json (CLAUDE_1 a CLAUDE_5, ~441 conversaciones). Fuente completa: `IRAM_gap_v4_1_a_v4_3_16_CERRADO_2026-06-11.md`. Reemplaza la cobertura parcial anterior.

**Hallazgo estructural del período:** las 5 cuentas trabajaban **en paralelo**, no secuencialmente — ver `cuentas_paralelas` en hitos v6 (Sección 4). El modelo de "cada cuenta continúa cuando la anterior llega al límite" no describe mayo de 2026: los 5 Claudes tenían sesiones IRAM activas el mismo mes, cada uno con un contexto parcial del proyecto.

### Pre-período — diseño base (contexto necesario)

| Fecha | Cuenta | Sesión | Resultado |
|---|---|---|---|
| 2026-05-11 | CLAUDE_4 | 'Diseño de decisiones de optimización para IRAM mod' (38 msgs) | Diseño on_action del Optimize ORIGINAL (17 rangos) — sistema v3, **no** Optimize Global |
| 2026-05-12 | CLAUDE_4 | 'Renombrar keys de decisiones en Exodos: Optimizar' (52 msgs) | Nomenclatura del optimize legacy cerrada |
| 2026-05-15 | CLAUDE_5 | 'Revisión de superbackup del ecosistema' (26 msgs) | Primera mención documentada de `iram_13` (ID de Optimize Global), ya planeado |
| ~2026-05-16 | CLAUDE_2 | 'Revisión integral de IRAM v4' (6 msgs) | Segunda mención de `iram_13` en el SUPERBACKUP |

### Período central (2026-05-22 → 2026-05-30)

**2026-05-22** — CLAUDE_1, sesión 'v4' (72 msgs) → backup v3.3/v3.4. Fix Gather Global capital exclusion (`NOT = { is_capital = yes }` en 21 bloques de `exodos_on_action.txt`), fix BOM-como-texto en `exodos_decisions_optimize.txt`, fix relics tokens (`picture`→delete, `gold`→`treasury`, `add_gold`→`add_treasury`). **No es la primera implementación de Optimize Global** — Optimize ya existía; esto es refinamiento. CLAUDE_2 corrió en paralelo una sesión corta de continuación ('v4.0', 6 msgs).

**2026-05-23** — CLAUDE_3, sesión '23/05/2026 2:48 am' (130 msgs). **Demografía — Ascenso/Descenso forzado.** La migración forzada ya estaba en el zip (`mod_pack_IRAM_15_GATHER_GLOBAL_v1_3.zip`, con `iram_decisions_migracion.txt`); esta sesión agregó `global_pop_promotion_speed` y `global_pop_demotion_speed` al modifier de migración existente. Trigger: pregunta del operador "¿y la velocidad de ascenso/descenso de pops?". Origen de la migración forzada en sí queda como deuda residual (anterior al 2026-05-22).

**2026-05-25/26** — CLAUDE_1, 'Actualización de documentación y logs de superbackup' (22 msgs) → v4.3.2, primer registro v4.3.x. Luego, sesión clave: **'Cálculo incorrecto de thresholds de esclavos' (50 msgs)** — **diseño final de Optimize Global**. Calibración del threshold por provincia: `(minas × 10) + (basic_settlement × 10) + (latifundia × 15)`; corrección de un error (base 15→20 para cities); diseño de `iram_script_values.txt` (dinámico, no hardcoded); operación en 2 pasos (validación de ancla + distribución por building). Diseño cerrado.

**2026-05-27 — día de alta densidad, múltiples cuentas en paralelo:**
- CLAUDE_2: 3 sesiones cortas 'Qué sigue' (0/12/32 msgs) → v4.3.4/5, coordinación post-corte
- **CLAUDE_2, 'Ejecución de tareas pendientes IRAM' (58 msgs)** → v4.3.6 — **diseño del Constructor Automático (`iram_12`)**. Error de nomenclatura descubierto: el fuerte real es `fortress_building`, no `hill_fort`. Generó **RE6** ("verificar nombre exacto de building en `00_default.txt` antes de codear")
- **CLAUDE_3, 'Prioridades del proyecto: constructor automático o reestructuración' (30 msgs)** → v4.3.7 — **sesión que origina el TECHNICAL_WIKI y el split ACTIVE/ARCHIVE** (ver hito en Sección 4 de hitos v6). Discusión sobre git history reconstruction. SUPERBACKUP v2.5 → v2.6
- CLAUDE_4: 2 sesiones de 'Implementación de iram_12_constructor_auto' (2/24 msgs) — stubs
- CLAUDE_5: 'Implementar constructor automático iram_12' (28 msgs) — más trabajo paralelo en iram_12

**2026-05-28**
- **CLAUDE_1, 'Falta bloque PASO 1 del prompt maestro' (46 msgs)** → v4.3.8 — **primera inicialización de Git** (`git init`, commit inicial = v4.3.7, "Historial completo en TECHNICAL_WIKI Secciones 14 y 19"). Diseño de `iram_11` (Distribute Global) e `iram_13` (Optimize Global on_action) en progreso; `iram_script_values.txt` confirmado en contexto
- CLAUDE_2, 'LOG NO COMPLETADO' → v4.3.9 (log truncado)
- CLAUDE_4, 'IRAM v4.3.8 carga completada' (6 msgs) — auditoría: `iram_11`, `iram_12`, `iram_13` son **stubs** en v4.3.8, código aún no escrito

**2026-05-29**
- **CLAUDE_3, 'Orden de ejecución para optimización iram_13' (104 msgs)**, desde v4.3.9 → v4.3.10+ — **implementación de Optimize Global (`iram_13`)**: limpieza de `exodos_gather_global_completed` (variable obsoleta + `on_yearly_pulse` eliminado + prep `exodos.3`), on_action con 5 bloques `monthly_country_pulse` (estructura idéntica a Distribute Global, lógica de slaves vía `iram_optimize_threshold`), textos finales ES/EN del ciclo IRAM
- CLAUDE_4, 'Sesión de análisis de archivos técnicos' → v4.3.10/11/12 (versiones intermedias)
- CLAUDE_1, 'Actualizar wiki técnico con logs de sesión' (22 msgs) → v4.3.11

**2026-05-30 — cierre de v4**
- CLAUDE_1, 'Tarea de sesión con archivos' (82 msgs) → **v4.3.13** — code review, cleanup de bloat en on_action, fixes de `trigger_event = { id = exodos.2 }` en Distribute Global y Optimize Global
- **CLAUDE_1, 'ESTADO ACTUAL 30/05' (50 msgs)** → **v4.3.14 / v4.3.15 / v4.3.16** — sesión de cierre de v4. 4 fixes finales (BUG A/B/C + estructural). **R19 agregada al PROMPT_MAESTRO** ("antes de modificar cualquier archivo: describir el cambio en una oración y esperar confirmación explícita del operador, sin excepción"), generada porque Claude ejecutó los 4 fixes sin confirmación previa. `exodos.3` eliminado en v4.3.14 (limpieza definitiva del mecanismo anual). **`mod_pack_IRAM_v4_3_16_2026-05-30_03-14.zip` — versión canónica final**, hora 03:14.

> **Corrección respecto a la nota de deuda original:** v4.3.16 no salió de 'Tarea de sesión con archivos' (82 msgs, → v4.3.13) sino de 'ESTADO ACTUAL 30/05' (50 msgs). Ambas son sesiones distintas de CLAUDE_1 el mismo día.

### Implementaciones — fechas y autoría confirmadas

**Optimize Global (`iram_13`):** diseño del threshold 2026-05-26 (CLAUDE_1); `iram_script_values.txt` registrado 2026-05-28; on_action implementado 2026-05-29 (CLAUDE_3, v4.3.10+). Autoría colaborativa — Operador diseñó el threshold, CLAUDE_3 implementó los 5 bloques.

**Constructor Automático (`iram_12`):** diseño 2026-05-27 (CLAUDE_2 + CLAUDE_4); error de nomenclatura `hill_fort`→`fortress_building` corregido en esa sesión; stub en v4.3.8; implementación completa entre v4.3.8 y v4.3.13 (versión exacta sin verificar). Autoría colaborativa.

**Distribute Global (`iram_11`):** diseño 2026-05-28 (CLAUDE_1); stub en v4.3.8. Autoría colaborativa.

**Demografía — Migración Forzada:** ya presente el 2026-05-23 en `mod_pack_IRAM_15_GATHER_GLOBAL_v1_3.zip`; sesión de origen anterior al 2026-05-22, no identificada (deuda residual).

**Demografía — Ascenso/Descenso Forzado:** primera evidencia 2026-05-23 (CLAUDE_3, 130 msgs); `global_pop_promotion_speed`/`global_pop_demotion_speed` agregados al modifier de migración existente, a partir de pregunta del operador. Autoría: Operador inició la idea, implementación colaborativa.

**SUPERBACKUP alcanzó 220KB** — el problema que detonó el split ACTIVE/ARCHIVE. El documento de referencia técnica creció mezclando instrucciones, contexto histórico, código fuente y decisiones. Claude no podía priorizar las reglas sepultadas en el contexto → repetía bugs documentados. El split ACTIVE/ARCHIVE (2026-05-27, CLAUDE_3) fue la respuesta directa — ver hito `technical_wiki_split` en hitos v6.
...
✅ **Gap cerrado 2026-06-11.** Fuente primaria: conversations.json CLAUDE_1 a CLAUDE_5 (~441 conversaciones), período analizado 2026-05-08 → 2026-05-30, 12 sesiones revisadas en profundidad.

**Deuda residual remanente** (no bloquea Plantillas D/B/C, pero queda pendiente):
- Fecha exacta de origen de la migración forzada (`iram_decisions_migracion.txt`) — anterior al 2026-05-22, buscar con ese keyword en CLAUDE_1-5
- Versión exacta donde `iram_11` (Distribute Global) dejó de ser stub — entre v4.3.9 y v4.3.13, revisar CLAUDE_3 2026-05-29 desde msg 35+
- Transiciones de cuenta CLAUDE_1→2, 2→3, 3→4, 4→5 (fechas exactas) — no resuelto por este análisis, que se enfocó en el gap técnico
- Reformular el modelo de "transiciones de cuenta" como gestión de contextos parciales en paralelo (ver hallazgo `cuentas_paralelas`, hitos v6 Sección 4)
...
## 3.1 Comparación v4.3.16 vs v5.5

| Dimensión | v4.3.16 | v5.5 |
|---|---|---|
| Estructura de mods | 1 carpeta `exodos/` con todo | 4 mods separados con contenido propio |
| Prefijo | `exodos_` + `iram_` mezclados | `iram_` consistente en todo |
| On_action | 1 archivo, 993 líneas, 3 monthly_pulse | 4 archivos especializados, 1.522 líneas total |
| Archivos de decisiones | 1 carpeta `exodos/decisions/` con todo | Cada mod tiene su propia carpeta `decisions/` |
| Menú exodos | 3 niveles anidados | 1 nivel: GG / DG / OG / Transfer / Constructor |
| Menú BOM | Mismo árbol que exodos | 2 niveles propios (Por otros medios / Demografía / Heredero) |
| Cancel | 1 cancel monolítico `exodos_cancel_all` | 1 cancel por mod — sin dependencias cruzadas |
| `potential` en decisiones | Ocultaba decisiones según condiciones de juego | Solo `is_ai = no` + variables de estado de menú |
| `allow` en decisiones | Condiciones sin tooltip descriptivo | `custom_tooltip` en español e inglés para todas las condiciones |

## 3.2 Estructura de archivos v5.5
...
├── exodos/
│   ├── common/
│   │   ├── modifiers/iram_relic_modifiers.txt
│   │   ├── on_action/
│   │   │   ├── iram_on_action_gather_global.txt      (597 líneas)
│   │   │   ├── iram_on_action_distribute_global.txt  (437 líneas)
│   │   │   ├── iram_on_action_optimize_global.txt    (403 líneas)
│   │   │   └── iram_on_action_transfer.txt           (85 líneas)
│   │   ├── script_values/iram_script_values.txt
│   │   ├── scripted_effects/iram_scripted_effects.txt
│   │   └── units/iram_units.txt
│   ├── decisions/
│   │   ├── iram_exodos_decisions.txt
│   │   ├── iram_exodos_cancel.txt
│   │   ├── iram_exodos_constructor.txt               (131 líneas — standalone)
│   │   ├── iram_compat_legacy.txt                    (124 líneas — compat saves)
│   │   ├── iram_events.txt                           (34 líneas)
│   │   └── [archivos de decisiones de exodos]
│   └── localization/
│       ├── english/ [archivos _l_english.yml]
│       └── spanish/ [archivos _l_spanish.yml]
│
├── by_other_means.mod
├── by_other_means/
│   ├── common/scripted_effects/iram_bom_scripted_effects.txt
│   ├── decisions/
│   │   ├── iram_bom_decisions.txt
│   │   ├── iram_bom_demografia.txt
│   │   ├── iram_bom_ego_sum.txt
│   │   ├── iram_bom_menu.txt                         (212 líneas)
│   │   └── iram_bom_rival_heir.txt                   (177 líneas)
│   └── localization/ [archivos bilingüe]
│
├── the_great_leap.mod
├── the_great_leap/
│   ├── decisions/ [archivos TGL]
│   └── localization/ [archivos bilingüe]
│
├── the_last_vote.mod
└── the_last_vote/
    ├── decisions/ [archivos TLV]
    ├── events/ [eventos TLV]
    └── localization/ [archivos bilingüe]
```

**Regla de rutas confirmada en v5.5:** `[mod]/decisions/` sin `common/` intermedio. El engine IR2.0.4 no carga desde `[mod]/common/decisions/`. La TECHNICAL_WIKI tenía documentada la ruta incorrecta — corregida en v5.x.

## 3.3 On_action en v5.5

| Archivo | Líneas | Operación |
|---|---|---|
| `iram_on_action_gather_global.txt` | 597 | Gather Global |
| `iram_on_action_distribute_global.txt` | 437 | Distribute Global |
| `iram_on_action_optimize_global.txt` | 403 | Optimize Global |
| `iram_on_action_transfer.txt` | 85 | Transfer |
| **Total** | **1.522** | — |

**Comparación con v4.3.16:** 993 líneas (1 archivo) → 1.522 líneas (4 archivos). El aumento de 53% en líneas se explica porque:
1. El refactoring de Optimize en v4.x (34KB → 10KB) había comprimido el código a expensas de claridad
2. En v5.0 se re-expandió con mejor estructura: cada operación es un archivo legible e independiente

## 3.4 Modularidad — regla de instalación

Cada mod (exodos, by_other_means, the_great_leap, the_last_vote) puede instalarse y desinstalarse sin afectar a los demás. La única restricción: los archivos `.mod` raíz y las carpetas raíz son intocables — cambiarlos afecta el `dlc_load.json` del jugador. Cualquier contenido dentro de esas carpetas puede reorganizarse libremente.

---

# ══════════════════════════════════════════════════════════
# SECCIÓN 4 — HITOS METODOLÓGICOS DEL PERÍODO v5
# ══════════════════════════════════════════════════════════

Los hitos de este período se documentan con las 5 dimensiones del sistema. Para el checklist completo del proyecto ver `IRAM_hitos_metodologicos_2026-06-10_v4.md`.

---

## 4.1 session_log_consolidado

**Fecha:** 2026-06-04 02:45 (CLAUDE_4)
**Autoría:** Colaborativo — el operador propuso dividir en sesión de diseño + sesión de ejecución; Claude generó el formato de 4 partes.

**Causalidad:**
*Antes hacíamos X:* SESSION_LOG con estado + próximos pasos simples.
*Generaba el problema Y:* para reworks con 13+ TAREAs y decisiones de diseño interdependientes, el SESSION_LOG simple no era suficiente instrucción para que otra IA ejecutara sin tomar decisiones. La IA ejecutora necesitaba certeza, no margen de interpretación.
*Por eso nació Z:* SESSION_LOG_CONSOLIDADO de 4 partes: (1) estado de ejecución por TAREA, (2) decisiones de diseño cerradas, (3) plan canónico con comandos exactos, (4) material de referencia (tooltips bilingüe, tabla de IDs).

**Ciclo de vida:** Temporal/situacional — para reworks de gran escala donde sesión de diseño y sesión de ejecución están separadas. No aplica a desarrollo cotidiano.

---

## 4.2 zips_wip_por_tarea (R19)

**Fecha:** 2026-06-06 (CLAUDE_3, durante ejecución TAREA 3)
**Autoría:** Operador-iniciativa — *"¿y los archivos por si se corta la sesión? ¿las tareas 1 y 2 se perdieron entonces?"*

**Causalidad:**
*Antes hacíamos X:* el zip de trabajo vivía solo en memoria del contenedor.
*Generaba el problema Y:* si la sesión se cortaba mid-task, todo el trabajo de las TAREAs completadas se perdía. No había checkpoint.
*Por eso nació Z:* zip WIP descargable después de cada tarea. Si la sesión se corta, el operador retoma desde el último WIP sin pérdida.

**Ciclo de vida:** Temporal/situacional — para reworks largos con múltiples TAREAs. No aplica a sesiones de desarrollo normal donde la IA entrega un solo zip al final.

**Nota:** esta regla nació del dolor, no del diseño. El operador perdió el trabajo de 2 TAREAs en una sesión cortada y preguntó qué había pasado. La respuesta fue la regla.

---

## 4.3 minilogs_por_tarea (R20)

**Fecha:** 2026-06-06 (CLAUDE_3, misma sesión que R19)
**Autoría:** Colaborativo — el operador pidió el minilog en .md; Claude propuso el formato estándar `IRAM_MINILOG_TAREA_X_[FECHA]_[HORA].md`; el operador lo adoptó como R20.

**Causalidad:**
*Antes hacíamos X:* el operador recibía el trabajo al final de la sesión como zip único sin registro intermedio.
*Generaba el problema Y:* cuando una sesión se cortaba mid-task, no había forma de saber exactamente qué había cambiado, qué archivos se tocaron, qué faltaba.
*Por eso nació Z:* minilog por tarea documenta: qué se cambió, qué archivos se modificaron, qué quedó pendiente. Sirve de base para el SESSION_LOG final y para retomar desde el WIP correspondiente.

**Ciclo de vida:** Temporal/situacional — para reworks largos de múltiples TAREAs. Los minilogs son más granulares que el SESSION_LOG (que es por sesión completa).

---

## 4.4 RD1 — Potential mínimo / Allow informativo

**Fecha:** 2026-06-04 (CLAUDE_4, sesión de diseño v5)
**Autoría:** Operador — *"el jugador tiene que tener acceso a la información siempre. para eso están las tooltips."*

**Causalidad:**
*Antes hacíamos X:* `potential` ocultaba decisiones cuando las condiciones de juego no se cumplían.
*Generaba el problema Y:* el jugador no sabía que esas funciones existían ni cuándo podría usarlas. El sistema de menús era invisible para el jugador desinformado.
*Por eso nació Z:* `potential` solo para `is_ai = no` + variables de estado de menú (qué nivel está abierto). Todas las condiciones de juego van a `allow`, con `custom_tooltip` descriptivos. Las decisiones que no se pueden usar aparecen en gris con tooltip explicativo — siempre visibles.

**Ciclo de vida:** Permanente — regla de diseño retroactiva: también se aplicó al código existente de demografía, ego_sum, y otros módulos que ocultaban decisiones.

---

# ══════════════════════════════════════════════════════════
# SECCIÓN 5 — DECISIONES DE DISEÑO TOMADAS EN ESTE PERÍODO
# ══════════════════════════════════════════════════════════

## 5.1 Sesión de diseño v5 — 8 decisiones (CLAUDE_4, 2026-06-04)

**Fuente:** conv "Confirmación de cambios en archivos IRAM" (75 msgs)

### D1 — Modularidad independiente

Cada mod puede instalarse y desinstalarse sin afectar a los demás. Restricción permanente: `.mod` files y carpetas raíz intocables. Dentro de esas carpetas: libertad total para reorganizar.

### D2 — Rename global exodos_ → iram_

Prefijo `exodos_` → `iram_` en todo el código: variables, scripted_effects, eventos, IDs de decisiones, archivos. El prefijo `iram_` era ya la convención para funciones nuevas — el rename unificó el namespace completo del proyecto. El nombre "exodos" quedó solo en el `.mod` file y la carpeta raíz (intocables por D1).

### D3 — Split on_action en 4 archivos

`exodos_on_action.txt` (993 líneas, 3 monthly_country_pulse) → 4 archivos especializados, 1 monthly_pulse cada uno. Riesgo "last wins" eliminado por arquitectura. Cada operación tiene su propio archivo: transfer, gather_global, distribute_global, optimize_global.

### D4 — Reorganización temática de contenido

Cada módulo vuelve a su mod semántico:
- BOM, ego_sum, demografía, rival_heir → `by_other_means/`
- TGL → `the_great_leap/`
- TLV + sus eventos → `the_last_vote/`
- exodos conserva solo: GG, DG, OG, Transfer, Constructor, Cancel, menú

### D5 — RD1: Potential mínimo, Allow informativo

Ver Sección 4.4. Aplica retroactivamente a todo el código existente.

### D6 — Menú exodos aplanado

El árbol de submenús de v4 (management → behavior → political) era innecesariamente profundo. En v5 el menú de exodos se aplana:

```
Abrir menú IRAM → GG / DG / OG / Transfer / Constructor
Cancel All IRAM (visible si hay operación activa)
```

### D7 — Cancel modular sin dependencias cruzadas

Cada mod tiene su propio cancel. Exodos limpia variables de exodos. BOM limpia variables de BOM/demografía/ego_sum. Cualquier combinación de mods instalados funciona correctamente.

### D8 — Ego Sum redesignado

Restricción doble:
- Una vez por dios por partida → variable en scope del país (persiste aunque cambie el gobernante)
- Solo una opción por gobernante → variable en scope del personaje (desaparece cuando el gobernante muere)
- Cancel resetea ambas variables — cancel como herramienta de control total

## 5.2 Apertura del período — temas que se resolvieron de v4.x

| Tema abierto en SUPERBACKUP v2.1 | Estado al cierre en v5.5 |
|---|---|
| Optimize Global — diseño | ✅ CERRADO — implementado en v4.x, refactorizado en v5.0 |
| Reliquia migración `global_migration_speed = 2.5` | ✅ CERRADO — implementado en v4.x |
| Condición `has_spouse` en Heredero del Rival | ⚠️ NO CONFIRMADO — pendiente de auditoría futura |
| `current_ruler` desde country scope en TLV | ⚠️ NO TESTEADO — registrado como pendiente |
| `valor_rp = 0.023223` | ⚠️ Sin verificar — no afecta código |

---

# ══════════════════════════════════════════════════════════
# SECCIÓN 6 — BUGS ENCONTRADOS Y RESUELTOS EN v5
# ══════════════════════════════════════════════════════════

## 6.1 Bug notable — potential mutuamente excluyente (todas las versiones anteriores)

`iram_exodos_menu_close` (y otras decisiones del árbol de submenús legacy) tenía en `potential`:

```pdxscript
potential = {
    has_variable = iram_exodos_menu
    NOT = { has_variable = iram_exodos_menu }
}
```

Condiciones mutuamente excluyentes — nunca podían ser verdaderas simultáneamente. La decisión era permanentemente invisible en pantalla. El bug había existido en **todas** las versiones anteriores del mod que usaban el árbol de submenús navegable.

**Resolución:** no se corrigió — se eliminó. La arquitectura de v5 (dos mods independientes con árboles propios) reemplazó el diseño donde el bug existía. Documentado para el ARCHIVE.

**Impacto metodológico:** un sistema de submenús que existió durante todo el proyecto (v1→v4) era funcionalmente muerto. Ninguna auditoría anterior lo detectó porque el bug silenciaba completamente la evidencia en pantalla.

## 6.2 Bug de carpeta — `common/decisions/` no existe en IR2.0.4

Durante el testing de v5 se descubrió que el engine no cargaba archivos en `[mod]/common/decisions/`. La ruta funcional en IR2.0.4 es `[mod]/decisions/` directamente.

**La TECHNICAL_WIKI tenía la ruta incorrecta documentada.** El engine la ignoraba silenciosamente — sin error.log, sin señal visible. El error se detectó porque en v5.0 se reorganizaron las carpetas y se probó exhaustivamente.

**Corrección:** todos los archivos de decisiones → `[mod]/decisions/` sin `common/` intermedio. Confirmado en zip v5.5 para los 4 mods.

**Impacto:** ERROR documentado en TECHNICAL_WIKI ACTIVE como ERROR 32 (o número asignado en la sesión de corrección).

## 6.3 Otros bugs del período v5.0→v5.5

El período v5.1→v5.5 fue de bugfixes post-testing. Los bugs específicos de ese período están documentados en:
- Los SESSION_LOGs de v5.1, v5.2, v5.3, v5.4, v5.5
- La AUDITORIA_CONSOLIDADA v5.5 (35 hallazgos)
- La TECHNICAL_WIKI_ACTIVE v3.10 Sección 0.5 (bugs pendientes)

---

# ══════════════════════════════════════════════════════════
# SECCIÓN 7 — LOG DE SESIONES CLAVE DEL PERÍODO
# ══════════════════════════════════════════════════════════

## 7.1 Índice de sesiones del período v4.x → v5.5
...
| 2026-05-27 | CLAUDE_3 | 'Prioridades del proyecto: constructor o reestructuración' | Sesión estratégica — ACTIVE/ARCHIVE split, nombre TECHNICAL_WIKI |
| 2026-05-27 | varios | 'Codear constructor automático' | Constructor implementado |
| 2026-05-30 | CLAUDE_1 | 'Tarea de sesión con archivos' (82 msgs) | v4.3.16 — última v4 canónica |
| 2026-06-04 | CLAUDE_4 | 'Confirmación de cambios en archivos IRAM' (75 msgs) | **Sesión de diseño v5 — 8 decisiones** |
| 2026-06-05 | CLAUDE_5 | TARs 0–1 | Wiki ACTIVE + PROMPT_MAESTRO actualizados |
| 2026-06-06 | CLAUDE_3 | 'Proyecto IRAM' (32 msgs) | TAREAs 2–6 ejecutadas — rebuild v5.0 + nacimiento R19/R20 |
| 2026-06-06 | varios | TAREAs 7–13 | Menú BOM, ID harmonization, ego_sum, tooltips |
| 2026-06-08 | CLAUDE_1 | 'IRAM v5.1 testing and error resolution' | v5.0 → v5.1 |
| 2026-06-08 | CLAUDE_2 | 'IRAM v5.2 prompt maestro setup' | v5.1 → v5.2 |
| 2026-06-09 | CLAUDE_1 | 'Organizar tareas pendientes de sesión' | v5.2 → v5.3 |
| 2026-06-09 | CLAUDE_2 | 'Carga de documentación técnica' (34 msgs) | v5.4 + auditoría consolidada |
| 2026-06-09 03:22 | CLAUDE_3 | 'Proyecto IRAM v5.5 — Discrepancias documentales' | v5.5 final — .mod versions + wiki cleanup |

## 7.2 Sesión estratégica 2026-05-27

**Estado:** ✅ CONFIRMADO — evidencia directa en ARCHIVE v3.7, Sección 19 (STRATEGIC LOG)

La sesión del 2026-05-27 no fue solo una sesión de trabajo técnico — fue la sesión donde se articuló explícitamente el doble objetivo del proyecto:
*"El mod exitoso es el entregable. El aprendizaje es el objetivo real."*

Esta sesión:
- Ejecutó el split ACTIVE/ARCHIVE (TECHNICAL_WIKI v3.0)
- Definió el nombre TECHNICAL_WIKI
- Documentó el STRATEGIC LOG con el perfil del operador y el mapa de skills desarrolladas
- Articuló las conexiones IRAM → ciencia de datos

**Cuenta exacta pendiente de verificar:** ⚠️ CLAUDE_3 o CLAUDE_4. Requiere conversations.json.

---

# ══════════════════════════════════════════════════════════
# SECCIÓN 8 — ESTADO DEL SISTEMA DE DOCUMENTACIÓN AL CIERRE
# ══════════════════════════════════════════════════════════

## 8.1 Sistema de tres capas al cierre (v5.5)

| Capa | Documento | Estado al cierre |
|---|---|---|
| PROMPT_MAESTRO | IRAM_PROMPT_MAESTRO v5.2 | ✅ Activo — incluye R19 y R20 |
| WIKI ACTIVE | TECHNICAL_WIKI_ACTIVE v3.10 | ✅ Activo — 1.630 líneas (limpiada en sesión 03:47) |
| SESSION_LOG | SESSION_LOG_v5_5_2026-06-09_03-22 | ✅ Generado — cierre del proyecto |
| WIKI ARCHIVE | TECHNICAL_WIKI_ARCHIVE v3.7 | ✅ Activo — 163KB, 3.476 líneas, historia completa v1→v4 + log desde 2026-05-19 |

## 8.2 Documentos de documentación disponibles

| Documento | Versión | Qué cubre |
|---|---|---|
| IRAM_SUPERBACKUP_v2_1.md | v2.1 | Historia v1→v4.0, código fuente v1/v2/v3/v4, decisiones del período |
| IRAM_BACKUP_ESTRATEGICO_v4_1_a_v5_5.md | (este documento) | Período v4.1→v5.5 — narrativa, arquitectura, hitos |
| IRAM_narrativa_v5_2026-06-10.md | v1 | Narrativa causal completa de v5 con evidencia directa |
| IRAM_hitos_metodologicos_2026-06-10_v4.md | v4 | Checklist de hitos con 5 dimensiones — completo hasta v5.5 |
| SESSION_LOG_DOCUMENTACION_2026-06-10_18-58.md | — | Log de sesión de documentación — hallazgos y mapa de versiones |

## 8.3 Herramientas temporales del período v5

Estas herramientas nacieron en el período v5 y son **Temporal/situacional** — no forman parte del sistema base del proyecto:

| Herramienta | Cuándo activar | Cuándo no activar |
|---|---|---|
| ZIP WIPs por tarea (R19) | Reworks de múltiples TAREAs en sesiones separadas | Desarrollo cotidiano con un zip al final |
| MINILOGs por tarea (R20) | Mismas condiciones que R19 — como complemento | Sesiones cortas de 1–3 cambios |
| SESSION_LOG_CONSOLIDADO | Reworks donde sesión de diseño y ejecución están separadas | Sesiones donde se diseña y ejecuta en la misma sesión |

---

# ══════════════════════════════════════════════════════════
# SECCIÓN 9 — TRANSICIONES DE CUENTA EN EL PERÍODO
# ══════════════════════════════════════════════════════════

⚠️ Esta sección requiere análisis sistemático de conversations.json para completarse.

| Transición | Fecha aprox. | Estado del sistema | Hitos asociados |
|---|---|---|---|
| → CLAUDE_3 | ⚠️ pendiente | ⚠️ pendiente | `primer_session_log` (concepto 2026-05-23) |
| → CLAUDE_4 | ⚠️ pendiente | SUPERBACKUP v2.1 + PROMPT_MAESTRO v3.x | Sesión de diseño v5 (2026-06-04) |
| → CLAUDE_5 | ⚠️ pendiente | PROMPT_MAESTRO v5.x + WIKI ACTIVE v3.x | TAREAs 0–1 ejecutadas |
