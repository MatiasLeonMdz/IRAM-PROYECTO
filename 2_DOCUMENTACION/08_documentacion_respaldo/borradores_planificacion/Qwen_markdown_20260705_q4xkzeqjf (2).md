...
- Qué es: El sistema de documentación de 5 capas (wikis ACTIVE/ARCHIVE, session logs, prompt maestro, backups)
...
- Propósito: Estudiar patrones de fricción, deuda técnica de interacción, y mecanismos de mitigación
...
Tarea 0.8: Definir conceptos descriptivos de fricción/deuda observados en el proyecto
- Herramienta: Discusión
- Entregable: Definiciones operativas y nombres finales
...
1. ¿Aceptó sin cambios? → SÍ = AT
2. ¿Cambió solo nombres/formato? → SÍ = ACCM
3. ¿Se mantiene arquitectura general? → SÍ = ACCS
4. ¿Hubo que restaurar backup? → SÍ = CS, NO = RE
...
**Conceptos descriptivos (a confirmar en Fase 0):**

Durante el desarrollo del proyecto IRAM, observamos dos patrones recurrentes en la colaboración humano-IA:

1. Costo acumulado de interacciones subóptimas (denominación provisorio: DTI): Situaciones donde la IA pierde contexto, propone soluciones ya descartadas, o genera trabajo que debe ser corregido posteriormente. Este patrón genera trabajo correctivo futuro que podría haberse evitado con mejor documentación o comunicación inicial.

2. Esfuerzo mental adicional del humano (denominación provisorio: FCC): Situaciones donde el operador debe re-explicar contexto, validar que la IA no alucine, reformular prompts malinterpretados, o frenar acciones no autorizadas. Este esfuerzo representa una carga cognitiva adicional al trabajo técnico propiamente dicho.

Estos patrones se documentarán descriptivamente en el análisis, sin pretensión de generalización teórica. El objetivo es observar cómo se manifiestan en este caso específico y qué mecanismos de mitigación (wikis, logs, prompt maestro) emergieron durante el proyecto.

**Denominación final:** A confirmar en Fase 0. Se evaluará si usar términos descriptivos simples o si conviene adoptar terminología de literatura existente (deuda técnica, fricción cognitiva) como marco de referencia.
...
Tarea 1.7: Rastreo de origen documental de patrones observados en los 5 JSONs
- Herramienta: Python + grep semántico
- Entregable: origen_patrones_observados.md
...
Tarea 4.5: Agregar sección S4B: Mecanismos de mitigación como respuesta adaptativa
- Herramienta: Edición
- Entregable: IRAM_C1_final_corregido.md
...
1. Origen de los mecanismos: Las herramientas de documentación (WIKI ACTIVE/ARCHIVE, SESSION_LOG, PROMPT_MAESTRO, MINILOG) no fueron diseñadas a priori. Emergieron como respuesta adaptativa a fricciones y deudas técnicas de interacción acumuladas durante el desarrollo.

2. Tabla de correspondencia tensión → respuesta:
...
- Sesiones cortadas: Se pierde trabajo al cortar sesión → SESSION_LOG + MINILOG → Primera aparición: [cita]
...
1. Documentación viva desde día 1: No esperar a que la fricción aparezca.
2. Separación ACTIVE/ARCHIVE: Conocimiento vigente e histórico en archivos separados.
3. Log de decisiones abiertas: Documentar explícitamente qué quedó sin resolver.
4. Reglas de agencia explícitas: Definir qué puede y qué no puede hacer la IA sin autorización.
5. Protocolo de corte de sesión: Instruir a la IA para sugerir generar log al acercarse al límite.
6. Auditorías periódicas: Comparar lo documentado vs lo implementado cada N sesiones.
...
- Fase 1: Procesamiento → CSV → Fecha límite: 2026-07-08 → Responsable: IA de bajo nivel
...
- Fase 1: tabla_analisis.csv + tabla_analisis_checksum.txt + origen_patrones_observados.md (formatos CSV + TXT + MD)
...
Los conceptos de [nombre final Concepto 1] y [nombre final Concepto 2] emergieron del proceso colaborativo documentado entre el investigador humano y el sistema de IA a lo largo de múltiples sesiones de desarrollo (ver historial unificado 2026-06-12). No fueron propuestos por un agente individual, sino que surgieron de la iteración continua donde el humano identificaba problemas reales de colaboración y la IA proponía soluciones que eran validadas, rechazadas o modificadas.
...
1. RAG (Retrieval-Augmented Generation): Busca información relevante automáticamente.
2. Vector Databases + Embeddings: Búsqueda semántica a escala.
3. Fine-tuning: Modelo especializado con documentos del proyecto.
4. Context Management Systems: Decide automáticamente qué contexto cargar.
5. Memory Layers: Mantiene memoria persistente entre sesiones.

**Sistema manual IRAM (5 capas):**

1. TECHNICAL_WIKI ACTIVE: Archivo markdown con todo lo vigente.
2. TECHNICAL_WIKI ARCHIVE: Archivo markdown con histórico.
3. PROMPT_MAESTRO: Instrucciones permanentes.
4. SESSION_LOG: Registro de cada sesión.
5. SUPERBACKUP: Archivo monolítico con todo el contexto.
...
**Próxima acción:** Iniciar Fase 0 con los materiales necesarios para resolver DR-54.