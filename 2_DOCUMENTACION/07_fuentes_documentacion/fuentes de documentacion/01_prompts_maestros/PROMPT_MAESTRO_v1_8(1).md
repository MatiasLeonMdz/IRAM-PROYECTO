# PROMPT — SISTEMA DE DOCUMENTACIÓN IRAM v1.8
...
Dos entregables derivados del mismo material:
  ...
  Estructura: 5 bloques (ver Plantilla C1).
...
9. IRAM_hitos_metodologicos_2026-06-12_v7.md — documento definitivo de hitos.
   ...
   ⚠️ pendientes: transiciones exactas C1→2, 2→3, 3→4, 4→5.
10. SESSION_LOG_DOCUMENTACION_2026-06-12_CONSOLIDADO_s9.md — log único de estado.
    Reemplaza todos los logs anteriores. Marco teórico completo (12 ángulos).
...
14. IRAM_SKILL_desarrollo_con_IA_v1_0.md — borrador del paper.
    ...
    ⚠️ PENDIENTE C1: restructurar para lector humano sin contexto del proyecto.
    ⚠️ PENDIENTE C2: extraer reglas prescriptivas (~40-60 líneas) para Claude.
...
[⚠️] rotacion_de_contextos — sistema multi-cuenta como solución al límite de tokens
      Causalidad: límite de msgs/cuenta → rotación forzada → PROMPT_MAESTRO como mecanismo de coherencia
      PENDIENTE: formalizar con fechas exactas de transición C1→2, 2→3, 3→4, 4→5
...
Historial unificado: IRAM_historial_unificado_2026-06-12.md ✅
                     441 convs | 7345 msgs post-dedup | 3.6MB
Análisis cuant.:     IRAM_analisis_cuantitativo_2026-06-12_v2.md ✅ (v1 obsoleta)
...
C1 — Paper:          ❌ PENDIENTE — restructurar SKILL.md para lector humano
...
Input: IRAM_SKILL_desarrollo_con_IA_v1_0.md (borrador — materia prima)
       IRAM_analisis_cuantitativo_2026-06-12_v2.md (datos cuantitativos)
       IRAM_gaps_conocimiento_[fecha].md
Output: IRAM_paper_metodologia_v1_0.md
...
1. Resumen ejecutivo
   Qué es el proyecto. Qué se encontró. Por qué importa.
   No más de 300 palabras. Sin jerga del proyecto.

2. El proyecto en números
   Snapshot cuantitativo como entrada: 441 convs, 7345 msgs, 5 cuentas,
   rotación secuencial (no paralela), velocidad por fase, evolución del contexto.
   Los datos establecen la escala antes de la narrativa.

3. Las dos historias en paralelo
   El mod se construía mientras el sistema de documentación emergía.
   Cada punto de corte del sistema (backup → SUPERBACKUP → PROMPT_MAESTRO →
   ACTIVE/ARCHIVE split → v5 modular) tiene un correlato en el mod.
   No mezclar las dos narrativas en el mismo párrafo.

4. Los hallazgos con evidencia
   Los 4 puntos de corte medibles: antes/después de cada cambio estructural.
   Efecto cuantificable de cada decisión. Esto convierte opinión en análisis.
   Principio central al cierre de este bloque.

5. Qué transfiere y qué no
   Las 3 condiciones de no-transferibilidad (Ángulo 11: criterio preexistente,
   árbitro claro, problema contenido). Con ejemplos concretos del proyecto.
   El paper termina con honestidad sobre los límites, no con optimismo genérico.
...
1. IRAM_paper_metodologia_v1_0.md
2. SESSION_LOG_DOCUMENTACION_[fecha]_CONSOLIDADO_s[N].md — log actualizado (único).
3. Respuesta a la pregunta de cierre (R14).
```
...
Input: IRAM_paper_metodologia_v1_0.md (C1 — paper completo)
Output: IRAM_skill_desarrollo_ia_v2_0.md
        (versión operacional — reemplaza v1.0 que era borrador del paper)
...
YAML FRONTMATTER OBLIGATORIO:
  ...
  description: [descripción precisa para triggering — cuándo Claude debe cargar este skill]
  version: 2.0

ESTRUCTURA — 6 SECCIONES PRESCRIPTIVAS

1. Cuándo usar este skill
   Condiciones de activación. Cuándo NO usarlo.

2. Arranque de sesión
   Qué hacer primero. Qué verificar. Qué cargar.
   Formato: lista de acciones, no explicaciones.

3. Durante la sesión
   Reglas de trabajo. Qué delegar a la IA, qué no.
   Patrones de error a evitar (los del paper, condensados en reglas).

4. Cierre de sesión
   Qué generar antes de terminar. Cómo actualizar el SESSION_LOG.
   La pregunta de cierre (R14).

5. Si la sesión falla o se corta
   Qué priorizar. Cómo retomar sin rework.

6. Principio de operación
   Una sola línea. El principio central condensado en instrucción para Claude.
...
Bloque 2 — Rotación de contextos ✅
  ...
  Metodología: timestamps de mensajes individuales (campo `ts`).
...
*PROMPT DOCUMENTACIÓN IRAM v1.8 — 2026-06-12*
...
*v1.5→v1.6: Plantilla A ejecutada. R19 agregada. Principio central incorporado.*
*v1.6→v1.7: R18 corregida — modelo "cuentas paralelas" reemplazado por "rotación
secuencial de contextos" (verificado con timestamps de mensajes individuales, 0
interleavings en 7.313 msgs). Nombre R18 actualizado. Hito cuentas_paralelas
movido a falsos positivos; reemplazado por rotacion_de_contextos. Plantilla D
marcada como ejecutada. Plantilla C sección 9 actualizada. Sesiones vacías
aclaradas en R8. Analisis cuantitativo v2 referenciado en DOCUMENTOS ACTUALES.*
*v1.7→v1.8: Plantilla C dividida en C1 (research narrative para humanos) y C2
(skill operacional para Claude). IRAM_SKILL_v1.0 reclasificado como borrador del
paper — no entregable final. Plantillas A, B y borrador C marcados como ejecutados.
SECUENCIA DE TRABAJO, DOCUMENTOS ACTUALES y ESTADO ACTUAL actualizados. Item 14
agregado en DOCUMENTOS ACTUALES. Notas ejecutadas en Plantillas A, B y D.*
