...
>
> **VERSIÓN _11 (2026-07-08, misma sesión) — agrega §18, dos precisiones sobre §17 que el operador dio en el turno siguiente.** El operador aclaró dos cosas que §17 no capturaba: (1) el criterio de diseño del árbol definitivo no es genérico — tiene que quedar legible y navegable específicamente para **instancias de IA simples y para tareas cortas** (no solo para sesiones largas de reconstrucción como las que viene haciendo esta cadena); (2) los errores de servicio de Anthropic del 6-7/07 (ya documentados en la respuesta de chat de la sesión anterior, no en ningún `§` hasta ahora) le costaron **un día entero de trabajo perdido**, por no poder ver/recuperar código generado a medias — dato de costo real que no estaba registrado en ningún documento persistente hasta esta sección. Ver §18 para el detalle y el ajuste de criterio que esto agrega a la tarea #19.

> **VERSIÓN _12 (2026-07-10) — fusiona §19 a §22, ejecutadas en sesiones posteriores como archivos sueltos sin integrar, y cierra la tarea #19 (árbol definitivo de documentación, prioridad #1 desde `_10`).** Entre `_11` (08-07) y esta versión hubo cuatro sesiones de ejecución que produjeron `SECCION_19_EJECUCION_TAREA_19.md`, `SECCION_20_PURGA_CUARENTENA.md` y `SECCION_21_ARBOL_REPLANTEOS.md` (las tres del 08-07, mismo día que `_11` pero sesiones posteriores) y una cuarta, `SECCION_22_UNIFICACION_2026-07-10.md` (10-07, sesión de unificación en `IRAM_UNIFICADO/`), pero ninguna se había fusionado al cuerpo de este documento — quedaron como archivos anexos sin referencia cruzada en `INDICE.md` ni en esta fuente de verdad. Esta versión las incorpora como §19-§22 reales (ver más abajo), sin reescribir una palabra de su contenido — se pegan tal como se escribieron, cotejadas contra los archivos `SECCION_*` originales antes de fusionar. La tarea #19 de §17.3/§18.3 queda **cerrada**: el árbol definitivo existe, está unificado en una sola carpeta (`IRAM_UNIFICADO/`), tiene índice raíz legible para IA simple/tarea corta (§18.1), y las 15+ copias sueltas de escritorio dejan de usarse — backup en adelante solo vía GitHub. Esta misma versión (`_12`) pasa a vivir en la raíz como fuente de verdad vigente; `_11` se archiva en `00_fuente_de_verdad/` junto con `_2` a `_10`, mismo criterio ya aplicado en cada versión anterior.
...
## 18. RECONCILIACIÓN CRÍTICA #11 — Dos precisiones del operador sobre la tarea #19: audiencia del árbol y costo real de los incidentes de servicio

**Disparador:** en el mismo turno en que se cerró §17, el operador agregó dos precisiones que cambian cómo debe ejecutarse la tarea #19, no si debe ejecutarse.

### 18.1 — Criterio de diseño: el árbol tiene que servir a IAs simples y tareas cortas, no solo a sesiones de reconstrucción larga

Hasta §17, la tarea #19 estaba planteada en términos de "orden" y "purga" sin especificar para quién debe ser legible el resultado. El operador precisó el criterio real: la estructura final tiene que ser navegable por **instancias de IA simples**, en **tareas cortas** — no solo por una sesión larga como las que viene produciendo esta cadena (`_2` a `_10`), que tiene margen para leer `plan.md` completo, hacer `grep` cruzado y reconstruir contexto perdido a mano.

Esto es una restricción de diseño concreta, distinta de "que esté ordenado":

- Un árbol "ordenado" pero que dependa de que quien lo lea sepa la historia completa de DR-01 a DR-54 para entender qué carpeta mirar **no cumple el criterio**, aunque esté prolijo.
- El árbol definitivo necesita, como mínimo: (a) nombres de carpeta autoexplicativos sin requerir contexto previo (`01_logs_replanteo` ya cumple esto; `_CUARENTENA_DUPLICADOS` no lo cumple del todo, porque no dice qué hacer con lo que hay adentro), (b) un único archivo-índice corto en la raíz que diga qué carpeta es la fuente vigente para qué tipo de pregunta, sin que haga falta leer esta fuente de verdad completa para saberlo, y (c) cero ambigüedad de "cuál copia es la real" para cualquier archivo — el problema exacto que expuso §17.1 con los `SESSION_LOG_REPLANTEO_*` duplicados.
- Consecuencia práctica para la ejecución de la tarea #19: no alcanza con mover archivos a carpetas — hay que producir, como parte de la misma tarea, un documento corto de "cómo leer este árbol" pensado para una sesión que arranca sin memoria y sin tiempo de reconstruir nada, la situación que representan tanto "una IA simple" como "una tarea corta".

### 18.2 — Costo real registrado: un día de trabajo perdido por los incidentes de servicio del 6-7/07

El operador confirmó que los errores de servicio de Anthropic documentados en la respuesta de chat de la sesión anterior (elevated errors el 6/07 y el 7/07, ver búsqueda web de esa sesión — no repetida acá porque no es contenido de este documento, es contexto de chat) le costaron **un día entero de trabajo** por no poder ver ni recuperar código que se había generado a medias cuando la sesión se cortaba. Este dato no estaba registrado en ningún documento persistente — vivía solo en el chat de la sesión anterior a esta fuente de verdad.

Se registra acá, con el mismo estándar de trazabilidad que ya usa este documento para hechos sin evidencia documental directa (mismo criterio que DR-43 y la tarea #7 sobre `claude_5`): **por relato directo del operador, sin log propio que lo respalde**, porque es información real sobre el costo del proyecto que de otro modo se perdería.

**Relación con la tarea #19:** este incidente no es un problema de estructura de documentación — es un problema de infraestructura de sesión (límite de contexto/tokens combinado con incidentes de servicio de Anthropic), ya diagnosticado en el chat de la sesión anterior. No genera una tarea nueva de §5 porque no es algo que este proyecto pueda resolver desde adentro — pero sí refuerza, con un costo concreto y medido (un día completo), por qué la tarea #19 importa: cuanto más liviana y clara sea la estructura de partida de cada sesión, menos contexto hace falta cargar para orientarse, y menos probabilidad hay de que una sesión se corte mid-stream antes de entregar algo. Un árbol pensado para "IA simple + tarea corta" (§18.1) reduce directamente el riesgo que causó la pérdida de este día.

### 18.3 — Tareas de §5 que esta sección resuelve o actualiza (append-only)

- **Tarea #19** (§17.3): sin cambio de estado (sigue "Abierta"), pero su alcance se precisa — el entregable de la tarea no es solo "árbol de carpetas ordenado", es "árbol de carpetas ordenado + índice legible para IA simple/tarea corta", según §18.1. Se actualiza la descripción de la tarea en esta sección, no se reescribe la fila original de §17.3.

### 18.4 — Estado de los archivos entregables

Esta misma versión (`FUENTE_DE_VERDAD_IRAM_2026-07-07_11.md`) es el único artefacto nuevo de esta sesión, y se entrega como adjunto explícito en el mismo turno en que se termina de escribir.
...
### §22.7 — Incorporación de las 3 herramientas de auditoría al árbol

Los tres scripts que produjeron el rescate de los 274 archivos (§22.1-§22.2) —
`1_descomprimir_copias.py`, `2_buscar_contenido_unico.py` y
`3_copiar_candidatos_valiosos.py` — vivían fuera del árbol del proyecto (en el
entorno de la sesión que ejecutó la auditoría), sin quedar integrados a
`IRAM_UNIFICADO/` en el volcado inicial de §22.1-§22.5. Se agregaron después,
en la misma sesión, a
`2_DOCUMENTACION/07_fuentes_documentacion/fuentes de documentacion/09_scripts_python/`
— mismo destino que ya tienen las demás herramientas de auditoría del proyecto
(`verificar_iram.py`, `comparar_copias_iram.py`), verificado que no colisionan
por nombre con ningún script ya existente ahí.

## §23 — Revisión general de duplicados sobre el árbol ya unificado (2026-07-10, misma sesión)

### §23.1 — Motivo

A pedido del operador, se auditó el árbol completo de `IRAM_UNIFICADO/` en
busca de duplicados que el chequeo de colisiones original (§22.1) pudiera
no haber capturado.

### §23.2 — Causa raíz encontrada

§22.1 comparó los 274 candidatos contra el repo activo **tal como estaba
antes de empezar a copiar**, pero nunca volvió a cruzar cada lote nuevo
contra las carpetas que la propia sesión iba creando en el camino. Resultado:
5 de las 6 carpetas nuevas creadas para el rescate resultaron ser, total o
parcialmente, contenido ya redundante con material que ya vivía organizado
en otras partes del árbol (`1_MOD/06_historial_desarrollo/`, `1_MOD/IRAM_legacy
v1 v2 v3 v4/`, `00_fuente_de_verdad/`, `00b_descartables/`,
`2_DOCUMENTACION/02_charlas_y_resumenes/`,
`2_DOCUMENTACION/07_fuentes_documentacion/`, o
`3_ANALISIS/04_prueba_fuga_memoria/`), verificado por hash SHA-256 normalizado.

### §23.3 — Corrección ejecutada

- `0_REPLANTEOS_Y_DECISIONES/03_rescatados_274/` (20 archivos, 100% redundante) → eliminada entera.
- `1_MOD/herramientas_backups_rescatados/` (3 archivos, 100% redundante) → eliminada entera.
- `2_DOCUMENTACION/08_documentacion_respaldo/historiales_completos/` (15 archivos) → eliminada entera.
- `2_DOCUMENTACION/08_documentacion_respaldo/reorganizaciones/` (4 archivos) → eliminada entera.
- `2_DOCUMENTACION/08_documentacion_respaldo/borradores_planificacion/` (5 archivos) → eliminada entera.
- `1_MOD/datos_config_mod_rescatados/` bajó de 8 a 5 archivos genuinos (los 3
  restantes — `tgl_decisions.txt`, `tlv_decisions.txt`, `tlv_events.txt` —
  resultaron duplicados exactos, con otro nombre, de archivos ya existentes
  en `1_MOD/IRAM_legacy v1 v2 v3 v4/mod_pack_IRAM_v4_1/mod_pack_v4_1/exodos/`).
- Única carpeta limpia desde el principio:
  `2_DOCUMENTACION/08_documentacion_respaldo/readmes_indices_historicos/`
  (se le quitaron 2 duplicados internos entre sí, quedando 9).

Verificado con un segundo escaneo completo del árbol tras la limpieza: cero
duplicados restantes en ninguna carpeta creada por esta sesión de rescate.

### §23.4 — READMEs actualizados como consecuencia

`2_DOCUMENTACION/08_documentacion_respaldo/README.md` reescrito para reflejar
que solo `readmes_indices_historicos/` sobrevivió; `1_MOD/datos_config_mod_rescatados/README.md`
creado (no existía) explicando qué quedó (5 de 8) y por qué; `INDICE.md`
corregido para no listar las carpetas ya eliminadas.

## §24 — Cierre del barrido de referencias rotas + renombrado de READMEs de carpeta (2026-07-10, continuación)

### §24.1 — Motivo

Quedaban dos tareas abiertas de la sesión anterior: (a) terminar el barrido
mecánico de rutas/nombres de archivo citados en todo el árbol, iniciado pero
no cerrado tras §23; y (b) resolver que los 11 READMEs de carpeta vigentes se
llamaban todos `README.md` a secas — indistinguibles entre sí si se
descargan sueltos fuera de su carpeta contenedora (problema detectado por el
operador al bajar copias de trabajo durante esta misma sesión).

### §24.2 — Barrido de referencias: resultado

Se extrajeron todas las rutas con forma de carpeta citadas entre backticks
en cualquier `.md` del árbol y se cruzaron contra el filesystem real. Todas
las menciones a rutas ya inexistentes encontradas resultaron estar dentro de
narrativa histórica ya cerrada (secciones append-only §19-§21 de este mismo
documento, o logs de auditoría como `LOG_CONTINUIDAD_IRAM_UNIFICADO_2026-07-10.md`
y `3_ANALISIS/03_auditoria_continuidad/`) — no se tocaron, porque reescribirlas
violaría la regla append-only (DR-42) y falsificaría el registro de lo que
efectivamente se hizo en su momento.

La única referencia colgada real, fuera de contexto histórico, era en el
propio `INDICE.md`: la sección "Contenido rescatado..." todavía mencionaba
`0_REPLANTEOS_Y_DECISIONES/03_rescatados_274/` y
`1_MOD/herramientas_backups_rescatados/`, ambas eliminadas en §23.3. Se
corrigió, reemplazando ese párrafo por el que ya reflejaba correctamente el
estado post-limpieza.

### §24.3 — Renombrado de READMEs de carpeta

Los 11 READMEs vigentes de primer/segundo nivel —hasta ahora todos con
nombre `README.md`— se renombraron incluyendo la ruta completa de la carpeta
que documentan, separada por doble guion bajo (mismo espíritu que ya usa
`readmes_indices_historicos/`, sin el hash final porque estos son únicos por
definición, no duplicados a diferenciar):

| Antes | Después |
|---|---|
| `00_fuente_de_verdad/README.md` | `README__00_fuente_de_verdad.md` |
| `00b_descartables/README.md` | `README__00b_descartables.md` |
| `0_REPLANTEOS_Y_DECISIONES/README.md` | `README__0_REPLANTEOS_Y_DECISIONES.md` |
| `1_MOD/README.md` | `README__1_MOD.md` |
| `2_DOCUMENTACION/README.md` | `README__2_DOCUMENTACION.md` |
| `2_DOCUMENTACION/07_fuentes_documentacion/README.md` | `README__2_DOCUMENTACION__07_fuentes_documentacion.md` |
| `2_DOCUMENTACION/08_documentacion_respaldo/README.md` | `README__2_DOCUMENTACION__08_documentacion_respaldo.md` |
| `3_ANALISIS/README.md` | `README__3_ANALISIS.md` |
| `3_ANALISIS/03_auditoria_continuidad/README.md` | `README__3_ANALISIS__03_auditoria_continuidad.md` |
| `_CUARENTENA_DUPLICADOS/README.md` | `README___CUARENTENA_DUPLICADOS.md` |

Los READMEs históricos de `readmes_indices_historicos/` (con patrón
`README__CARPETA__hash.md`) no se tocaron: ya cumplían el objetivo.

Las menciones a `README.md` dentro de narrativa histórica (§19, §20, §21 de
este documento, y logs de auditoría) tampoco se tocaron por la misma razón
de §24.2 — describen nombres válidos en el momento que narran.

### §24.4 — Estado del `.git`

Esta sesión trabajó sobre un árbol reconstruido combinando el `.git` real
del operador (que solo tenía commiteado el estado previo a toda esta
unificación, del 2026-07-08) con el contenido final correcto del árbol
post-unificación y post-limpieza (§22-§23), que nunca había llegado a
commitearse. Los cambios de §22, §23 y esta sección (§24) quedan como
working tree sin commitear — el operador decide cuándo consolidar esto en
un commit único de git.

### §24.5 — Pendiente para la próxima sesión

- Commitear el estado resultante de §22-§24 en git (nunca se hizo).
